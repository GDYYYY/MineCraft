Shader "Unlit/MyBRDFShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float4 mainTex = tex2D( _MainTex, i.uv ); // texture color
				float4 normalTex = tex2D( _NormalMap, i.uv);

                //// Vectors
                float3 L = normalize(lerp(_WorldSpaceLightPos0.xyz, _WorldSpaceLightPos0.xyz - i.worldPos.xyz,_WorldSpaceLightPos0.w));// light direction
                float3 V = normalize(_WorldSpaceCameraPos.xyz - i.worldPos.xyz);// view direction
                float3 H = Unity_SafeNormalize(L + V);// half vector direction
